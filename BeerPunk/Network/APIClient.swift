...
final class APIClient: APIService {
    ...
    private lazy var jsonDecoder: JSONDecoder = {
        ...
        jsonDecoder.userInfo[CodingUserInfoKey.managedObjectContext] = StorageProvider.shared.persistentContainer.viewContext
        return jsonDecoder
    }()
...
final class MockedAPIClient: APIService {
    ...
    func search(for name: String) async throws -> [Beer] {
        Beer.demoData.filter { beer in
            guard let name = beer.name else {
                return false
            }
